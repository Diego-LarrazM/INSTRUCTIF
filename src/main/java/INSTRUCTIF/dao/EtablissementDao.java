...
public class EtablissementDao {
    ...
    public List<EtabHitInstance> getHitList(){
        return JpaUtil.obtenirContextePersistance().createQuery("SELECT new INSTRUCTIF.util.EtabHitInstance(e.id, e.lat, e.lng, count(elv)) from Etablissement e LEFT JOIN Eleve elv GROUP BY e.id, e.lat, e.lng ORDER BY count(elv) DESC ",EtabHitInstance.class).getResultList();
    }
