...
public class Scenarios {
    ...
    private static void ChronologicalEventsAlice() {
    ...
    private static Soutien CreationSoutien(Eleve e) {
        Soutien soutien = null;
        System.out.println("\n[PHASE] Creation d'un Soutien par " + e.getPrenom());
        List<Matiere> matieres = Serv.obtenirMatieres();
        if(matieres.size() > 0){
            soutien =  new Soutien("Ticher said That mon Englishe is-e very bad, I need-e help ?", matieres.get(0));
    ...
    private static boolean DemandeSoutien(Eleve e, Soutien s) {
        ...
        boolean res = Serv.demanderSoutien(e, s);
        if(res) {
